...
/**
 * __cgroup_bpf_run_filter_lsm_filepermission - Run a program on LSM file_permission hook
 *
 * @file: the file structure being accessed
 * @mask: the requested permissions
 *
 * Program is run when the file_permission hook is triggered.
 * The hook is triggered when checking file permissions before accessing and open file.
 *
...
int __cgroup_bpf_run_filter_lsm_filepermission(struct file *file, int mask)
{
	...
	rcu_read_lock();
	cgrp = task_dfl_cgroup(current);
	ret = BPF_PROG_RUN_ARRAY(cgrp->bpf.effective[BPF_CGROUP_LSM_FILEPERMISSION], &ctx, BPF_PROG_RUN);
	rcu_read_unlock();

	return ret == 1 ? 0 : -EPERM;
}
EXPORT_SYMBOL(__cgroup_bpf_run_filter_lsm_filepermission);

/**
 * __cgroup_bpf_run_filter_lsm_fileopen - Run a program on LSM file_open hook
 *
 * @file: the file structure being accessed
 *
 * Program is run when the file_open hook is triggered.
 * The hook is triggered when saving open-time permission checking states for
 * later use upon file_permission,
 * and rechecking if anything has changed since inode_permission.
 *
 * This function will return %-EPERM if an attached program is found but
 * returned value != 1 during execution. In all other cases 0 is returned.
 */
int __cgroup_bpf_run_filter_lsm_fileopen(struct file *file)
{
	...
	rcu_read_lock();
	cgrp = task_dfl_cgroup(current);
	ret = BPF_PROG_RUN_ARRAY(cgrp->bpf.effective[BPF_CGROUP_LSM_FILEOPEN], &ctx, BPF_PROG_RUN);
	rcu_read_unlock();
...
static bool cgroup_lsm_is_valid_access(int off, int size, enum bpf_access_type type,
				   const struct bpf_prog *prog,
				   struct bpf_insn_access_aux *info)
{
	const int size_default = sizeof(__u32);

	if (off < 0 || off + size > sizeof(struct bpf_cgroup_lsm_ctx) ||
	    off % size)
		return false;
...
const struct bpf_verifier_ops cgroup_lsm_verifier_ops = {
	.get_func_proto		= cgroup_lsm_func_proto,
	.is_valid_access	= cgroup_lsm_is_valid_access,
};
