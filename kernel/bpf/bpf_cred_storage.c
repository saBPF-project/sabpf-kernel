...
void bpf_cred_storage_free(struct cred *cred)
{
	...
	hlist_for_each_entry_safe(selem, n, &local_storage->list, snode) {
		...
		free_cred_storage = bpf_selem_unlink_storage_nolock(
...
static void *bpf_pid_cred_storage_lookup_elem(struct bpf_map *map, void *key)
{
...
static int bpf_pid_cred_storage_update_elem(struct bpf_map *map, void *key,
...
static int bpf_pid_cred_storage_delete_elem(struct bpf_map *map, void *key)
{
	struct task_struct *task;
	unsigned int f_flags;
	struct pid *pid;
...
BPF_CALL_4(bpf_cred_storage_get, struct bpf_map *, map, struct cred *,
	   cred, void *, value, u64, flags)
{
	struct bpf_local_storage_data *sdata;
	...
	/* This helper must only be called from places where the lifetime of the cred
...
const struct bpf_map_ops cred_storage_map_ops = {
	...
	.map_lookup_elem = bpf_pid_cred_storage_lookup_elem,
	.map_update_elem = bpf_pid_cred_storage_update_elem,
	.map_delete_elem = bpf_pid_cred_storage_delete_elem,
